...
class PvPImmunityValidationTask implements Runnable {
	...
	public void run() {
		...
		if (!GriefPreventionPlus.isInventoryEmpty(this.player)) {
			// if found, cancel invulnerability and notify
			playerData.pvpImmune = false;
			GriefPreventionPlus.sendMessage(this.player, TextMode.Warn, Messages.PvPImmunityEnd);
		} else {
			// otherwise check again in one minute
			GriefPreventionPlus.getInstance().getServer().getScheduler().scheduleSyncDelayedTask(GriefPreventionPlus.getInstance(), this, 1200L);
		}
	}
}
